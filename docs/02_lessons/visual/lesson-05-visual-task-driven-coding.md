...
课程仓库里的样例工程可能已经显示部分任务为 `DONE`，这是因为样例工程本身已经完成。

学员跟做自己的项目时，应从 `TODO` 开始：

```text
TODO → DOING → REVIEW → FIXING → DONE
```
...
| TASK-004 | 实现本地收藏服务 | TODO | 使用 wx 本地存储 |
...
```text
TASK-001 到 TASK-006 已经是 DONE。
```

这代表样例工程已经跑通过，不代表学员一开始就应该全部设为 DONE。
