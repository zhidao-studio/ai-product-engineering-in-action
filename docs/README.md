# 文档导航

本目录只放说明性文档。

模板、检查清单、提示词、示例分别放在仓库顶层对应目录，不放进 docs。

## 1. 目录职责

```text
docs/REPOSITORY_STRUCTURE.md  仓库结构说明
docs/methodology/             方法论：目标、原则、分层、构造方法、覆盖检查
docs/engineering/             工程基线：技术选择、分层规则、质量红线
docs/lessons/                 教学化内容：后续面向非技术人的课程化表达
```

## 2. 阅读顺序

```text
1. docs/REPOSITORY_STRUCTURE.md
2. docs/methodology/01_GOAL_AND_SCOPE.md
3. docs/methodology/02_CORE_PRINCIPLES.md
4. docs/methodology/03_HARNESS_LAYERS.md
5. docs/methodology/04_HOW_TO_BUILD_HARNESS.md
6. docs/methodology/05_COVERAGE_CHECK.md
7. docs/engineering/01_ENGINEERING_BASELINE.md
8. docs/engineering/02_ENGINEERING_QUALITY_GUARDRAILS.md
```

## 3. 关联目录

```text
templates/harness/        harness 模板
templates/code_delivery/  代码交付模板
checklists/harness/       harness 检查清单
checklists/code_delivery/ 代码交付检查清单
prompts/                  AI / Agent / Review 提示词
examples/                 填好的示例
```
